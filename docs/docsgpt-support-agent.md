...
| `support-gateway` | 本仓库根目录 | 是 | 浏览器唯一访问的 AI 客服 API。 |
| `frontend-dashboard` | `sub2api` 仓库 | 是 | LumioAPI 前端。 |
...
| 文件 | 说明 |
|------|------|
| `server.go` | `/widget-config`、`/chat/stream`、CORS、限流、DocsGPT SSE 转发、语言透传。 |
| `config.go` | `DOCSGPT_API_BASE_URL`、`DOCSGPT_AGENT_API_KEY`、`ALLOWED_ORIGINS` 等环境变量加载。 |
| `Dockerfile` | Zeabur 部署 gateway 的镜像。 |
| `sub2api/frontend-dashboard/src/components/support/SupportChatWidget.vue` | 右下角客服气泡，读取当前 `locale` 并发送给 gateway。 |
| `sub2api/frontend-dashboard/src/api/supportChat.ts` | 获取 widget config，解析 SSE 流。 |
